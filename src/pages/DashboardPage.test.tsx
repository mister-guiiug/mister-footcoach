...
describe('DashboardPage', () => {
  ...
  it('shows @ for away upcoming match', () => {
    const awayMatch = {
      ...
      location: 'Away Stadium', isHome: false, opponent: 'FC Away Test', status: 'saison' as const,
      phase: 'Test', liveActive: false,
    };
    localStorage.setItem('mister-footcoach-data', JSON.stringify({
      ...MOCK_DATA, matches: [awayMatch], selectedTeamId: MOCK_DATA.teams[0]!.id,
    }));
    renderWithProviders(<DashboardPage />);
  ...
  it('shows "Entraînement" when training has no theme', () => {
    const noThemeTraining = {
      ...
      duration: 90, type: 'regulier' as const, cancelled: false,
    };
    localStorage.setItem('mister-footcoach-data', JSON.stringify({
      ...MOCK_DATA, trainings: [noThemeTraining], selectedTeamId: MOCK_DATA.teams[0]!.id,
    }));
    renderWithProviders(<DashboardPage />);
  ...
  it('shows singular "sondage" when exactly one survey is open', () => {
    localStorage.setItem('mister-footcoach-data', JSON.stringify({
      ...MOCK_DATA, surveys: [MOCK_DATA.surveys[0]], selectedTeamId: MOCK_DATA.teams[0]!.id,
    }));
    renderWithProviders(<DashboardPage />);
  ...
  it('shows training with unknown teamId without team name', () => {
    const orphanTraining = {
      id: 'tr99', teamId: 'unknown-team', date: '2026-05-20', time: '18:00',
      duration: 90, type: 'regulier' as const, cancelled: false, theme: 'Technique',
    };
    localStorage.setItem('mister-footcoach-data', JSON.stringify({
      ...MOCK_DATA, trainings: [orphanTraining], selectedTeamId: MOCK_DATA.teams[0]!.id,
    }));
    renderWithProviders(<DashboardPage />);
  ...
  it('excludes inactive players from team count', () => {
    const inactivePlayer = { ...MOCK_DATA.players[0], id: 'p-inactive', active: false };
    localStorage.setItem('mister-footcoach-data', JSON.stringify({
      ...MOCK_DATA, players: [...MOCK_DATA.players, inactivePlayer], selectedTeamId: MOCK_DATA.teams[0]!.id,
    }));
    renderWithProviders(<DashboardPage />);
    expect(screen.getByText('Mes équipes')).toBeInTheDocument();
  });
  ...
  it('hides upcoming trainings section when no upcoming trainings exist', () => {
    ...
    renderWithProviders(<DashboardPage />);
    expect(screen.queryByText('Prochains entraînements')).not.toBeInTheDocument();
  });

  it('hides tournaments section when all tournaments are terminated', () => {
    const terminatedTournament = { ...MOCK_DATA.tournaments[0], status: 'termine' as const };
    localStorage.setItem('mister-footcoach-data', JSON.stringify({
      ...MOCK_DATA, tournaments: [terminatedTournament], selectedTeamId: MOCK_DATA.teams[0]!.id,
    }));
    renderWithProviders(<DashboardPage />);
  ...
  it('shows team name as empty when match has unknown teamId', () => {
    const orphanMatch = {
      id: 'm88', teamId: 'unknown-team', seasonId: 's1', date: '2026-05-20', time: '15:00',
      location: 'Stadium', isHome: true, opponent: 'FC Orphan', status: 'saison' as const,
      phase: 'Test', liveActive: false,
    };
    localStorage.setItem('mister-footcoach-data', JSON.stringify({
      ...MOCK_DATA, matches: [orphanMatch], selectedTeamId: MOCK_DATA.teams[0]!.id,
    }));
    renderWithProviders(<DashboardPage />);
